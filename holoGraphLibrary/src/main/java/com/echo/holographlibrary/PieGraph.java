...
public class PieGraph extends View implements  HoloGraphAnimate {
    ...
    private Paint mPercentColorPaint = new Paint();
    ...
    public PieGraph(Context context, AttributeSet attrs, int defStyle) {
        ...
        mPercentColorPaint.setColor(getResources().getColor(android.R.color.white));
        mPercentColorPaint.setTextSize(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 12, getResources().getDisplayMetrics()));
        mPercentColorPaint.setTypeface(Typeface.DEFAULT_BOLD);
        mPercentColorPaint.setTextAlign(Paint.Align.CENTER);
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (PieSlice slice : mSlices) {
            ...
            p.arcTo(mRectF, (currentAngle + mPadding) + (currentSweep - mPadding), -(currentSweep - mPadding));
            p.close();
